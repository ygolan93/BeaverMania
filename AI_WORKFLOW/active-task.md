...
> **Status (2026-09-05, both Hurricane aerial attacks):** Mixed ownership; Codex script/test work complete, Cursor owns final visual/contact verification. Both `HurricaneKick` and `HurricaneSword` immediately trigger boss Reverse at the existing 12 speed / 0.6 seconds. Damage multipliers, Tree Stun, and Stunned/Dead guards are preserved. The existing `Base Layer.Backwards` animation starts once, bypassing Walk's slow exit transition; subsequent hits do not restart it. Verification: Scorpion EditMode **94/94 passed**; selected Scorpion + victory PlayMode job **succeeded, 10 completed, zero reported failures** (detailed result payload unavailable). No serialized asset edits were authored for this follow-up. See the latest Codex-to-Cursor handoff for final in-scene checks.

> **Status (2026-09-05, Scorpion boss combat contract VERIFIED in Unity; balance decisions pending):** Cursor-owned Unity Editor integration plus the **first actually-executed** test verification on `feature/ScorpionBoss_Rework` (HEAD `74c82bf6`). This closes the "0 EditMode and 0 PlayMode tests were executed" gap carried by every Scorpion status entry below. Run live via Unity MCP against `BeaverProject@d3b470b190d10be0` (Unity 2021.3.3f1): **Scorpion EditMode contract suites 48/48 passed**, **Scorpion boss + victory-flow PlayMode 8/8 passed**. Full EditMode suite ran 111 tests with 6 failures, **none Scorpion** (3 `AudioRoutingEditModeTests`, 3 `InputReaderLifecycleTests`). Every checklist item has executed-test evidence except respawn/fight-restart, which has no test. Unity integration delivered: `canStunScorpionBoss: 1` on `Assets/Prefabs/Objects/Interactable/TheGivingTree.prefab` (+3 lines) and the five approved values on `Assets/Data/NPC/Scorpion/ScorpionBossStatsData.asset` (+5 lines: `bossStunDuration 3.5`, `bossStunCooldown 10`, `stunnedDamageMultiplier 2`, `hurricaneKickRetreatSpeed 12`, `hurricaneKickRetreatDuration 0.6`). **No C# and no formulas changed.** Both files remain uncommitted: a stale `.git/index.lock` blocked the intended separate Unity-wiring commit and the user chose to commit manually.
>
> **Design model superseded:** `ScorpionVulnerabilityWindowTests.cs` has been removed and replaced by `ScorpionCombatContractTests.cs`. `ReceiveDamage_AdvancedBossInNormalState_IsAlwaysDamageable` now asserts the advanced boss is damageable in Idle/Attack/Charge/Reverse/Look. The punish-window model described in the 2026-08-26 entries below is therefore **no longer the shipped design** — it is now "always damageable, 2x while stunned". The deferred-lethal-hazard latch and the still-serialized `reverseVulnerabilityDuration: 0.8` should be re-read against that change; nothing appears to gate on the latter any more.
>
> **Blocked on Codex/User decisions (see `cursor-to-codex.md`, 2026-09-05):** (1) **P1 C# defect** — `AnimatedAttack.GroundAttack()` maps only `WeaponCategory.ArmorSet` to `PlayerAttackKind.SwordSwing`, so Hammers and Bow fall through to `Unspecified` (x1.0). Hammers therefore deals 700 per ground swing versus the Armor Set's 600 (200 x 3.0), inverting weapon progression and making the approved `SwordSwing` x3 unreachable by any other weapon. (2) **P2 balance** — a 3.5 s stun at 2x is worth roughly 2,400 damage, only **1.6 % of `maxHealth: 150000`**; with a 13.5 s cycle floor and only **25 `TheGivingTree` instances scene-wide** (each consumed on use, arena count unverified), the stun mechanic is currently marginal. Computed time-to-kill is 215–250 connected swings, roughly 6–8 minutes of real fight. Lowering `maxHealth` to ~40,000–60,000 is the recommended lever; it was outside the approved tuning range so it was left unchanged.
>
> **Blocks PlayMode CI (pre-existing, not Scorpion):** `WaspQueenBossPlayModeTests.Charge_LocksDirectionAtStart` now **hangs to the 180 s timeout** and aborted the unscoped PlayMode run at 36/111 — a regression versus the 2026-06-30 PASS recorded in `cursor-to-codex.md`. `PooledProjectile_DetachesFromBossHierarchy_AndKeepsWorldVelocity` also fails (1 vs 0 active projectiles), consistent with the open pool-parenting issue. Full PlayMode additionally pulls in the Input System package's own `IntegrationTests.WindowsInput_*` cases and should be filtered in CI.
>
> **Still needs manual Play Mode:** respawn and fight restart; a timed kill to check the duration arithmetic; how many Giving Trees actually sit within charge range of the boss arena; and the retreat's visual read — `isAttacking` is true during the covering reverse (contract passes) but the Animator plays `Backwards` with `Attack` false, so no claw motion is visible. Cursor could not hand-play the fight because player input cannot be injected through MCP.
