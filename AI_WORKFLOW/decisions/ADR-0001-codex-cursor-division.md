...
Beavermania is a Unity project with fragile serialized references and high coupling between script behavior and Unity Editor wiring (prefabs/scenes/UI/Animator/Inspector). Prior guidance defined tool strengths, but lacked a strict operational protocol for cross-agent lifecycle ownership and handoffs.

## Decision

Codex and Cursor cooperate through repository files under `AI_WORKFLOW/` instead of direct freeform conversation.

Codex is treated as the senior architect and codebase guardian.
Cursor is treated as the hands-on implementation engineer.
...
- Architecture decisions, system boundaries, focused refactor strategy, code reviews, risk notes, and PR summaries.
...
- Focused implementation, local editing, and bounded execution tasks.
...
- Generate Cursor -> Codex handoffs when Unity investigation indicates script-level defects or the task expands beyond safe local scope.
...
- Cursor acting as the default architect for ambiguous or risky work.
